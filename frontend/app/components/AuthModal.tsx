...
export function AuthModal({ onClose, onSuccess }: AuthModalProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const body =
			tab === "login" ? { email, password } : { username, email, password };
	...
	return (
		<Dialog open onOpenChange={(open) => !open && onClose()}>
			<DialogContent className="sm:max-w-100 bg-card/95 border-border/40 shadow-2xl backdrop-blur-2xl rounded-2xl animate-in">
