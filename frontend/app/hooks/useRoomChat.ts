...
export function useRoomChat({
  ...
  const [presenceUsers, setPresenceUsers] = useState<{ user_id: string; username: string }[]>([]);
  ...
  const toggleReaction = useCallback(
    (messageId: string, emoji: string) => {
      ...
      setReactionsById((prev) => {
        ...
        return { ...prev, [messageId]: [...current, { emoji, count: 1, reacted_by_me: true }] };
      });
      ...
      fetch(`${httpBase}/api/reactions`, {
        ...
        body: JSON.stringify({ message_id: messageId, room_id: activeRoomId, emoji }),
      }).catch(() => {
